/**
 * HubSpot Form Router - Standalone Version
...
// Import and initialize the HubSpot Router from the bundle
(function() {
  'use strict';
  
  // Load the main bundle if not already loaded
  if (!window.HubSpotRouterModule) {
    const script = document.createElement('script');
    script.src = 'dist/bundle.js';
    script.async = false; // Load synchronously to ensure proper initialization
    script.onload = function() {
      console.log('[Scheduler] Modular bundle loaded successfully');
    };
    script.onerror = function() {
      console.error('[Scheduler] Failed to load modular bundle');
    };
